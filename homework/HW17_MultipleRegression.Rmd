...
The correlation matrix shows that income, percent with a bachelor’s degree, and median age are the variables most strongly related to voter turnout. Higher income, higher education, and older populations tend to have higher turnout. Overall, the strongest linear predictors are income, education, and median age, while population and percent unemployed don’t show much of a relationship at all.
...
The model seems to predict turnout pretty well, but not perfectly. The residuals look fairly even around zero, which means the model isn’t making big, obvious mistakes. The R-squared shows that the model explains a decent amount of the variation in turnout.Overall, the model is useful, but it could be better. It helps us understand the main factors that influence turnout, but dropping the weak predictors would likely improve it.
...
After creating three alternative models, the adjusted R-squared values change only slightly. When we remove weaker predictors (like population or percent unemployed), the adjusted R-squared stays almost the same, showing that those variables weren’t helping the model much.

```{r}
# Model 2
model2 <- lm(turnout ~ income + pct_bachelors_higher + median_age, data = md_turnout)
summary(model2)
...
# Model 4
model4 <- lm(turnout ~ income + pct_bachelors_higher, data = md_turnout)
summary(model4)
...
You could use this analysis to explain why some counties vote more than others. By looking at which factors are linked to higher turnout—like income, education, or age—you can show readers what might be influencing participation.
