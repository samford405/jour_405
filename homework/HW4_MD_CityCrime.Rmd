...
ANSWER HERE: Most of the trends i see are that lots of the rates go down which makes sense because that was Covid but they re are a few rates that got even higher so I think a interesting story here could be on those places whose numbers got higher despite all of us being on lockdown
...
ANSWER HERE: This kinda proves the data from earlier with most cities rates going down because of Covid but places you wouldn't automatically suspect went up.
...
ANSWER HERE: 
Lede:
When the pandemic emptied streets and shut down nightlife, crime dropped in many Maryland cities but, in a few surprising places, it climbed instead.

Nutgraf:
The numbers tell a story of contrast: while most communities saw violent and property crime dip during 2020, several smaller cities, places not usually linked with rising crime, went against the trend. These unexpected spikes suggest the pandemic didn’t just reduce opportunities for crime, it reshaped them in uneven ways.
...
Yes it does because my theory was that in this are or certain area saw an uprise in violent crime but i an this code for 2019 it was actually a bit higher then compared to 2020

```{r}

md_cities_percentiles <- md_cities_change |>
  summarize(
    IQR_violent_crime_2020 = IQR(violent_rate_2020)
